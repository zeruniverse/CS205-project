...
void image_mul_scalar(image_t *image, const float scalar) {
    int i;
    float *imp = image->data;
    const float scalarp = scalar;
...
image_t *image_resize_bilinear(const image_t *src, const float scale) {
    ...
    const int newwidth = (int) (1.5f + (width - 1) /
                                       scale); // 0.5f for rounding instead of flooring, and the remaining comes from scale = (dst-1)/(src-1)
...
static void
convolve_extract_coeffs(const int order, const float *half_coeffs, float *coeffs, float *coeffs_accu, const int even) {
    int i;
    float accu = 0.0;
    if (even) {
        for (i = 0; i <= order; i++) {
...
// INSPECT this function takes 0% of total time.
...
static void convolve_horiz_fast_5(image_t *dst, const image_t *src, const convolution_t *conv) {
    ...
    for (j = 0; j < src->height; j++) {
        ...
        float *srcp_p1 = (float *) src_p1, *srcp_p2 = (float *) src_p2, *srcp_m1 = (float *) src_m1, *srcp_m2 = (float *) src_m2;
...
void color_image_convolve_hv(color_image_t *dst, const color_image_t *src, const convolution_t *horiz_conv,
                             const convolution_t *vert_conv) {
    const int width = src->width, height = src->height, stride = src->stride;
    // separate channels of images
    image_t src_red = {width, height, stride, src->c1}, src_green = {width, height, stride, src->c2}, src_blue = {width,
                                                                                                                  height,
                                                                                                                  stride,
                                                                                                                  src->c3},
            dst_red = {width, height, stride, dst->c1}, dst_green = {width, height, stride, dst->c2}, dst_blue = {width,
                                                                                                                  height,
                                                                                                                  stride,
                                                                                                                  dst->c3};
...
// INSPECT this function is only called twice.
...
color_image_pyramid_create(const color_image_t *src, const float scale_factor, const int min_size, const float spyr) {
