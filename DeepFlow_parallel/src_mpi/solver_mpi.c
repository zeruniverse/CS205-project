    ...
    for (int iter = 0; iter < iterations; iter++) {
        ...
        for (int j = 0; j < du->height; j++) {
            ...
            for (int i = 0; i < du->width; i++) {
                ...
                sigma_v = 0.0f;
                if (j > 0) {
                    sigma_u -= dpsis_vert->data[(j - 1) * du->stride + i] * to_u[(j - 1) * du->stride + i];
                    sigma_v -= dpsis_vert->data[(j - 1) * du->stride + i] * to_v[(j - 1) * du->stride + i];
                }
                if (i > 0) {
                    sigma_u -= dpsis_horiz->data[j * du->stride + i - 1] * to_u[j * du->stride + i - 1];
                    sigma_v -= dpsis_horiz->data[j * du->stride + i - 1] * to_v[j * du->stride + i - 1];
                }
                if (j < du->height - 1) {
