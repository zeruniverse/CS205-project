...
#include <string.h>
#include "opticalflow_aux_rbsor.h"
...
void get_derivatives(const color_image_t *im1, const color_image_t *im2, const convolution_t *deriv,
                     ...
                     color_image_t *dxx, color_image_t *dxy, color_image_t *dyy, color_image_t *dxt,
                     color_image_t *dyt) {
...
image_t *compute_smoothness_weight(color_image_t *im, float coef, const convolution_t *deriv) {
    image_t *lum = image_new(im->width, im->height), *lum_x = image_new(im->width, im->height), *lum_y = image_new(
            im->width, im->height);
    int i;
    // ocompute luminance
...
void compute_smoothness_mpi(image_t *dst_horiz, image_t *dst_vert, const image_t *uu, const image_t *vv,
                            const image_t *dpsis_weight, const convolution_t *deriv_flow, const float half_alpha) {
    int w = uu->width, h = uu->height, s = uu->stride;
    ...
    //MARK loop combined here
    for (int j = 0; j < h - 1; j++) {
        for (int i = 0; i < w - 1; i++) {
            ux1->data[j * s + i] = uu->data[j * s + 1 + i] - uu->data[j * s + i];
            vx1->data[j * s + i] = vv->data[j * s + 1 + i] - vv->data[j * s + i];

            uy1->data[j * s + i] = uu->data[j * s + s + i] - uu->data[j * s + i];
            vy1->data[j * s + i] = vv->data[j * s + s + i] - vv->data[j * s + i];
        }
        uy1->data[j * s + w - 1] = uu->data[j * s + s + w - 1] - uu->data[j * s + w - 1];
        vy1->data[j * s + w - 1] = vv->data[j * s + s + w - 1] - vv->data[j * s + w - 1];
    }
    int offset;
    for (int i = 0; i < w - 1; i++) {
        ux1->data[(h - 1) * s + i] = uu->data[(h - 1) * s + 1 + i] - uu->data[(h - 1) * s + i];
        vx1->data[(h - 1) * s + i] = vv->data[(h - 1) * s + 1 + i] - vv->data[(h - 1) * s + i];
    }
    ...
    for (j = 0; j < h; j++) {
        ...
        for (i = 0; i < w - 1; i++, offset++) {
            ...
            tmp = uxsq + vxsq;
            dst_horiz->data[offset] = (dpsis_weight->data[offset] + dpsis_weight->data[offset + 1]) * half_alpha /
                                      sqrtf(tmp + epsilon_smooth);
        }
    ...
    for (j = 0; j < h - 1; j++) {
        ...
        for (i = 0; i < w; i++, offset++) {
            ...
            tmp = uysq + vysq;
            dst_vert->data[offset] = (dpsis_weight->data[offset] + dpsis_weight->data[offset + s]) * half_alpha /
                                     sqrtf(tmp + epsilon_smooth);
        }
    }
...
void
compute_data_and_match(image_t *a11, image_t *a12, image_t *a22, image_t *b1, image_t *b2, image_t *mask, image_t *wx,
                       image_t *wy, image_t *du, image_t *dv, image_t *uu, image_t *vv, color_image_t *Ix,
                       color_image_t *Iy, color_image_t *Iz, color_image_t *Ixx, color_image_t *Ixy, color_image_t *Iyy,
                       color_image_t *Ixz, color_image_t *Iyz, image_t *desc_weight, image_t *desc_flow_x,
...
void
compute_desc_weight(const color_image_t *im1, const color_image_t *im2, const image_t *desc_x, const image_t *desc_y,
                    image_t *desc_w, const convolution_t *deriv_autocor, const convolution_t *deriv_flow) {
    ...
    color_image_t *sim1 = color_image_new(w, h), *sim2 = color_image_new(w, h),
            *im1dx = color_image_new(w, h), *im1dy = color_image_new(w, h), *im2dx = color_image_new(w,
                                                                                                     h), *im2dy = color_image_new(
            w, h);
    ...
    for (j = 0; j < h; j++) {
        for (i = 0, o = j * s; i < w; i++, o++) {
            if (desc_w->data[o] > 0.0f) {
                ...
                float flowscore =
                        abs(sim1->c1[o] - sim2->c1[o2]) + abs(sim1->c2[o] - sim2->c2[o2]) +
                        abs(sim1->c3[o] - sim2->c3[o2])
                        + grad_weight * (
                                abs(im1dx->c1[o] - im2dx->c1[o2]) + abs(im1dx->c2[o] - im2dx->c2[o2]) +
                                abs(im1dx->c3[o] - im2dx->c3[o2]) +
                                abs(im1dy->c1[o] - im2dy->c1[o2]) + abs(im1dy->c2[o] - im2dy->c2[o2]) +
                                abs(im1dy->c3[o] - im2dy->c3[o2]));
                float t2 = tmp->data[o];
    ...
    // free memory
    color_image_delete(sim1);
    color_image_delete(sim2);
    color_image_delete(im1dx);
    color_image_delete(im1dy);
    color_image_delete(im2dx);
    color_image_delete(im2dy);
    image_delete(dx2);
    image_delete(dxy);
    image_delete(dy2);
    image_delete(tmp);
...
void descflow_resize(image_t *dst_flow_x, image_t *dst_flow_y, image_t *dst_weight, const image_t *src_flow_x,
    ...
    const float scale_x = ((float) dst_width - 1) / ((float) src_width - 1), scale_y =
            ((float) dst_height - 1) / ((float) src_height - 1);
    image_erase(dst_flow_x);
    image_erase(dst_flow_y);
    image_erase(dst_weight);
    int j;
        ...
        for (i = 0; i < src_width; i++) {
            ...
            if (dx) {
                if (dy) {
                    ...
                    dst_flow_x->data[y2 * dst_stride + x2] =
                            (dst_flow_x->data[y2 * dst_stride + x2] * dst_weight->data[y2 * dst_stride + x2] +
                             src_flow_x->data[j * src_stride + i] * weightxy * scale_x) / newweight;
                    dst_flow_y->data[y2 * dst_stride + x2] =
                            (dst_flow_y->data[y2 * dst_stride + x2] * dst_weight->data[y2 * dst_stride + x2] +
                             src_flow_y->data[j * src_stride + i] * weightxy * scale_y) / newweight;
                    dst_weight->data[y2 * dst_stride + x2] = newweight;
                }
                weightxy = weight * dx * (1.0f - dy);
                newweight = dst_weight->data[y1 * dst_stride + x2] + weightxy;
                dst_flow_x->data[y1 * dst_stride + x2] =
                        (dst_flow_x->data[y1 * dst_stride + x2] * dst_weight->data[y1 * dst_stride + x2] +
                         src_flow_x->data[j * src_stride + i] * weightxy * scale_x) / newweight;
                dst_flow_y->data[y1 * dst_stride + x2] =
                        (dst_flow_y->data[y1 * dst_stride + x2] * dst_weight->data[y1 * dst_stride + x2] +
                         src_flow_y->data[j * src_stride + i] * weightxy * scale_y) / newweight;
            ...
            if (dy) {
                ...
                dst_flow_x->data[y2 * dst_stride + x1] =
                        (dst_flow_x->data[y2 * dst_stride + x1] * dst_weight->data[y2 * dst_stride + x1] +
                         src_flow_x->data[j * src_stride + i] * weightxy * scale_x) / newweight;
                dst_flow_y->data[y2 * dst_stride + x1] =
                        (dst_flow_y->data[y2 * dst_stride + x1] * dst_weight->data[y2 * dst_stride + x1] +
                         src_flow_y->data[j * src_stride + i] * weightxy * scale_y) / newweight;
            ...
            dst_flow_x->data[y1 * dst_stride + x1] =
                    (dst_flow_x->data[y1 * dst_stride + x1] * dst_weight->data[y1 * dst_stride + x1] +
                     src_flow_x->data[j * src_stride + i] * weightxy * scale_x) / newweight;
            dst_flow_y->data[y1 * dst_stride + x1] =
                    (dst_flow_y->data[y1 * dst_stride + x1] * dst_weight->data[y1 * dst_stride + x1] +
                     src_flow_y->data[j * src_stride + i] * weightxy * scale_y) / newweight;
...
void descflow_resize_nn(image_t *dst_flow_x, image_t *dst_flow_y, image_t *dst_weight, const image_t *src_flow_x,
                        const image_t *src_flow_y, const image_t *src_weight) {
    const int src_width = src_flow_x->width, src_height = src_flow_x->height, src_stride = src_flow_x->stride,
            dst_width = dst_flow_x->width, dst_height = dst_flow_x->height, dst_stride = dst_flow_x->stride;
    const float scale_x = ((float) dst_width - 1) / ((float) src_width - 1), scale_y =
