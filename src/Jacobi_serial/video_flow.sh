...
while true; do
  ...
  if [ -a $file2 ] && [ "$j" -le "$endFrame" ]; then
    if [ ! -f flow/forward_${i}_${j}.flo ]; then
      ./deepflow2 "$file1" "$file2" "flow/forward_${i}_${j}.flo" -match "match/forward_${i}_${j}.match"
    fi
    if [ ! -f flow/backward_${j}_${i}.flo ]; then
      ./deepflow2 "$file2" "$file1" "flow/backward_${j}_${i}.flo" -match "match/backward_${j}_${i}.match"
    fi
