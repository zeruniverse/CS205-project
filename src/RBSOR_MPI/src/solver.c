...
void rbsor_step(int red,
                float *du, float *dv,
                float omega,
                int W, int H, int stride,
                float *dph, float *dpv,
                float *A11m, float *A12m, float *A22m,
                float *b1d, float *b2d) {
    MPI_Status status;
	...
	//printf("\nI'm Master Node\n");
	    ...
	    //MPI_Send(&stride,1,MPI_INT,dest,tag,MPI_COMM_WORLD);
            if (dest == 1){
                ...
                MPI_Send(du,(rows+1)*stride,MPI_FLOAT,dest,tag,MPI_COMM_WORLD);
                MPI_Send(dv,(rows+1)*stride,MPI_FLOAT,dest,tag,MPI_COMM_WORLD);
                MPI_Send(dpv,rows*stride,MPI_FLOAT,dest,tag,MPI_COMM_WORLD);
            } else if (dest == numworker){
                mark = 2;
                MPI_Send(&mark,1,MPI_INT,dest,tag,MPI_COMM_WORLD);
                MPI_Send(du+(offset-1)*stride,(rows+1)*stride,MPI_FLOAT,dest,tag,MPI_COMM_WORLD);
                MPI_Send(dv+(offset-1)*stride,(rows+1)*stride,MPI_FLOAT,dest,tag,MPI_COMM_WORLD);
                MPI_Send(dpv+(offset-1)*stride,(rows+1)*stride,MPI_FLOAT,dest,tag,MPI_COMM_WORLD);
            }
            else{
                mark = 0;
                MPI_Send(&mark,1,MPI_INT,dest,tag,MPI_COMM_WORLD);
                MPI_Send(du+(offset-1)*stride,(rows+2)*stride,MPI_FLOAT,dest,tag,MPI_COMM_WORLD);
                MPI_Send(dv+(offset-1)*stride,(rows+2)*stride,MPI_FLOAT,dest,tag,MPI_COMM_WORLD);
                MPI_Send(dpv+(offset-1)*stride,(rows+1)*stride,MPI_FLOAT,dest,tag,MPI_COMM_WORLD);
            }
	...
	//printf("\nI'm a Master, and I finish assigning jobs\n");
        tag = FROM_WORKER;
        for (int i = 1;i<=numworker;i++){
            ...
            MPI_Recv(du+offset*stride,rows*stride,MPI_FLOAT,source,tag,MPI_COMM_WORLD,&status);
            MPI_Recv(dv+offset*stride,rows*stride,MPI_FLOAT,source,tag,MPI_COMM_WORLD,&status);
        }
	...
	//MPI_Recv(&stride,1,MPI_INT,0,tag,MPI_COMM_WORLD,&status);
        ...
        if (mark == 0){
            MPI_Recv(du,(rows+2)*stride,MPI_FLOAT,0,tag,MPI_COMM_WORLD,&status);
            MPI_Recv(dv,(rows+2)*stride,MPI_FLOAT,0,tag,MPI_COMM_WORLD,&status);
            MPI_Recv(dpv,(rows+1)*stride,MPI_FLOAT,0,tag,MPI_COMM_WORLD,&status);
	    du = du+stride;
	    dv = dv+stride;
	    dpv = dpv+stride;
        } else if (mark == 1){
            MPI_Recv(du,(rows+1)*stride,MPI_FLOAT,0,tag,MPI_COMM_WORLD,&status);
            MPI_Recv(dv,(rows+1)*stride,MPI_FLOAT,0,tag,MPI_COMM_WORLD,&status);
	...
	dpv = dpv-stride;
        ...
        if (mark == 1){
            ...
            MPI_Send(du+stride,rows*stride,MPI_FLOAT,0,tag,MPI_COMM_WORLD);
            MPI_Send(dv+stride,rows*stride,MPI_FLOAT,0,tag,MPI_COMM_WORLD);
        }
	//printf("I'm a worker, %d, and I finish sending message\n",rank);
    }
}
